...
<div class="container-fluid pet-grid">
    <div class="container">
        <h2 class="text-center mb-4">Hewan yang Hilang</h2>
        
        <div class="row">
            <?php 
            if (!empty($laporan)): 
                foreach ($laporan as $item): 
                    if ($item['status_admin'] === 'yes' && $item['tipe_laporan'] === 'Hilang'): 
            ?>
                <div class="col-md-4 mb-4">
                    <div class="pet-card" data-toggle="modal" data-target="#petModal<?= $item['id_laporan'] ?>">
                        <img src="<?= filter_var($item['foto_hewan'], FILTER_VALIDATE_URL) ? 
                            $item['foto_hewan'] : 
                            base_url('uploads/' . $item['foto_hewan']) ?>" 
                            class="w-100 pet-img" alt="<?= esc($item['nama_hewan']) ?>">
                        <div class="pet-info">
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Detail Hewan Hilang</h5>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <img src="<?= filter_var($item['foto_hewan'], FILTER_VALIDATE_URL) ? 
                                            $item['foto_hewan'] : 
                                            base_url('uploads/' . $item['foto_hewan']) ?>" 
                                            class="w-100 rounded" alt="<?= esc($item['nama_hewan']) ?>">
                                    </div>
                                    <div class="col-md-6">
                                        <h4><?= esc($item['nama_hewan']) ?></h4>
                                        ...
                                        <p><strong>Lokasi Terakhir:</strong> <?= esc($item['lokasi_terakhir']) ?></p>
                                        <p><strong>Kontak:</strong> <?= esc($item['nomor_telepon']) ?></p>
            ...
            ?>
                <div class="col-12 text-center">
                    <p>Tidak ada hewan hilang yang terdaftar.</p>
                </div>
            <?php endif; ?>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('.copy-number').click(function() {
            ...
            btn.attr('data-original-title', 'Tersalin!').tooltip('show');
            
            // Reset tooltip after 2 seconds
            setTimeout(() => {
                btn.attr('data-original-title', 'Salin nomor telepon').tooltip('hide');
            }, 2000);
